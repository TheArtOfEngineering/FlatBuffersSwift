...
private func set<T : Scalar>(_ buffer : UnsafeMutablePointer<UInt8>, _ objectOffset : Offset, propertyIndex : Int, value : T) throws {
    let propertyOffset = getPropertyOffset(buffer, objectOffset, propertyIndex: propertyIndex)
    if propertyOffset == 0 {
        throw FlatBufferReaderError.CanOnlySetNonDefaultProperty
    }
...
private func setVectorScalarElement<T : Scalar>(_ buffer : UnsafeMutablePointer<UInt8>, _ vectorOffset : Offset, index : Int, value : T) {
    let valueStartPosition = Int(vectorOffset + strideof(Int32) + (index * strideof(T)))
    var v = value
    let c = strideofValue(v)
    withUnsafePointer(&v){
...
func getListCountFrom(_ buffer : UnsafePointer<UInt8>, fooBarContainerOffset : Offset) -> Int {
    let offset_list : Offset? = getOffset(buffer, fooBarContainerOffset, propertyIndex: 0)
    return getVectorLength(buffer, offset_list)
}

func getFooBarOffsetFrom(_ buffer : UnsafePointer<UInt8>, fooBarContainerOffset : Offset, listIndex : Int) -> Offset {
    let offset_list : Offset? = getOffset(buffer, fooBarContainerOffset, propertyIndex: 0)
    return getVectorOffsetElement(buffer, offset_list!, index: listIndex)!
}

func getInitializedFrom(_ buffer : UnsafePointer<UInt8>, fooBarContainerOffset : Offset) -> Bool {
    return get(buffer, fooBarContainerOffset, propertyIndex: 1, defaultValue: false)
}
func getFrootFrom(_ buffer : UnsafePointer<UInt8>, fooBarContainerOffset : Offset) -> Enum {
    return Enum(rawValue: get(buffer, fooBarContainerOffset, propertyIndex: 2, defaultValue: Enum.Apples.rawValue))!
}
func getLocationFrom(_ buffer : UnsafePointer<UInt8>, fooBarContainerOffset : Offset) -> UnsafeBufferPointer<UInt8> {
    return getStringBuffer(buffer, getOffset(buffer, fooBarContainerOffset, propertyIndex: 3))!
}
func getLocationFromS(_ buffer : UnsafePointer<UInt8>, fooBarContainerOffset : Offset) -> String {
    return getString(buffer, getOffset(buffer, fooBarContainerOffset, propertyIndex: 3))!
}

func getSiblingFrom(_ buffer : UnsafePointer<UInt8>, fooBarOffset : Offset) -> Bar {
    let result : Bar = get(buffer, fooBarOffset, propertyIndex: 0)!
    return result
}
func getNameFrom(_ buffer : UnsafePointer<UInt8>, fooBarOffset : Offset) -> UnsafeBufferPointer<UInt8> {
    return getStringBuffer(buffer, getOffset(buffer, fooBarOffset, propertyIndex: 1))!
}
func getNameFromS(_ buffer : UnsafePointer<UInt8>, fooBarOffset : Offset) -> String {
    return getString(buffer, getOffset(buffer, fooBarOffset, propertyIndex: 1))!
}
func getRatingFrom(_ buffer : UnsafePointer<UInt8>, fooBarOffset : Offset) -> Float64 {
    let result : Float64 = get(buffer, fooBarOffset, propertyIndex: 2)!
    return result
}
func getPostfixFrom(_ buffer : UnsafePointer<UInt8>, fooBarOffset : Offset) -> UInt8 {
